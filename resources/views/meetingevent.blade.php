...
<script type="text/javascript" src="{{ asset('meetingjs/meetingrecorder.js') }}"></script>
<script type="text/javascript" src="{{ asset('meetingjs/meetingwrp.js') }}"></script>
<script type="text/javascript" src="{{ asset('meetingjs/meetingresult.js') }}"></script>
  ...
  <body>
    ...
    <form action="{{ url('meeting/edit') }}" method="POST">
      @csrf
        <textarea id="meetingResult"
            class="textarea w-full bg-gray-800 text-white text-9xl font-bold overflow-hidden p-8 border-none focus:outline-none"
            readonly
            style="height: 8em; /* 高さを調整して2行分のスペースを確保 */ line-height: 2em; /* 行間の調整 */">
        </textarea>
        <textarea id="recognitionResult" name="recording" class="textarea w-full font-bold p-8" style="height: 200px; overflow-y: scroll;">
        </textarea>
...
// meetingResultのテキストを更新
function updateMeetingTextarea(text) {
    var textareaElement = document.getElementById("meetingResult");
...
// resultFinalizedの修正: 認識結果を各テキストエリアに表示
function resultFinalized(result) {
    TRACE("EVENT: resultFinalized()");  // ログの出力
    result = Result.parse(result);
    if (result.text) {
        var text = sanitize_(result.text);

        // recognitionResultに全テキストを表示
        var recognitionTextarea = document.getElementById("recognitionResult");
        recognitionTextarea.value += text + "\n"; // 新しいテキストを追加

        // meetingResultに最新3行を表示
        updateMeetingTextarea(text); // meetingResultのテキストを更新
    }
}
